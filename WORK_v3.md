...
- [x] F2 방 만들기 **기본 6명**. (자리 막기(seat-blocking)는 후속 — 아래 후속 항목.)
- [x] F3 비공개방 비밀번호 + 공개/비공개 토글 라벨 개선(코드 시작 제거).
- [x] F4 초대 링크 공유(복사) + 딥링크 입장(?room=CODE). (네이티브 share 시트는 후속.)

## G. 계정·랭킹·프로필
- [x] G1 랭킹 계정별(uid 키, 동일인 1행).
- [x] G2 닉네임 변경권 10000골드(첫 설정 무료, 이후 변경권 소모) + 설정 경로 게이트. 익명은 랭킹 미등록.
...
## 후속(다음 세션)
- F2 **자리 막기(seat-blocking)**: 호스트가 특정 좌석을 막아 인원 제한(최소 2자리 못 막음). RTDB blocked-seats + 대기실 UI + computeView 반영 필요.
- F4 **네이티브 공유 시트**(share_plus, 카톡 직접): 현재는 링크 복사로 대체.
- D2 좌석에 연막 남은 횟수 배지(현재 액션바만).
- 비공개 방 비밀번호 서버 강제(현재 클라 소프트 게이트 — rooms read 공개).
