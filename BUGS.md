...
- [ ] (P2) 부두 저주 재시전이 도화선 10턴으로 리셋되는 버그 — 이미 저주 걸린 대상에 다시 걸면 curseFuse를 10으로 덮어써 죽음을 늦춤. 수정: **이미 저주 중(curseFuse[target]>0)이면 재시전 무효**(도화선 유지). party_logic.dart 저주 적용부(~585행 `curseFuse[m.target]=kCurseFuse` 앞에 `if(curseFuse[m.target]>0) continue` 가드). 단위테스트(이미저주→재시전 무효·도화선 불변) 추가. 화면: party_logic.dart
- [ ] (P1) 승률 트래킹 사이트(cowboy.gg) 작동 안 함 — 원인 조사·수정. **유력 원인**: 보안세션이 users read를 본인만으로 막았는데 charstats(공개 read여야 함)까지 영향받았거나, RTDB read 경로/포맷/.indexOn 문제. cowboy.gg repo의 데이터 로드 코드 + cowboy_party database.rules.json charstats read 규칙 확인. 화면: cowboy.gg repo, cowboy_party/database.rules.json
- [ ] (P2) 게임 폰트를 Pretendard로 교체 — OFL 오픈폰트. Pretendard(가능하면 variable 또는 필요 weight) 다운로드→`assets/fonts/`, pubspec `fonts:` 등록, theme.dart 기본 fontFamily 교체(한글 가독성↑). CREDITS.md에 OFL 표기. 화면: pubspec.yaml, theme.dart, assets/fonts, CREDITS.md
- [ ] (P2) 배경 선인장·태양 이미지 개선 — 현재 코드드로잉이 조악함. 더 발전된 비주얼로(개선된 코드 드로잉 또는 **CC0/퍼블릭도메인 무료 에셋** 다운로드해 적용, 라이선스 CREDITS 표기). 사막 노을 톤 유지. 화면: 배경 위젯(DesertBackground 등)
- [ ] (P2) 플레이 화면 "처음이세요? 게임방법…" 위치·디자인 개선 — 2줄 넘침·위치 부적절. **첫 실행 사용자에겐 팝업으로 안내**, 재방문자는 다른 곳(설정/작은 아이콘)에서 찾게. "튜토리얼 봤음" 플래그(meta/SharedPrefs) 사용. 화면: play_tab.dart, how_to_play_screen.dart, meta
