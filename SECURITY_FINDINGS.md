...
## 2026-07-12 추가 점검 (API 키·연결 전수 감사)
- ✅ **고위험 시크릿 커밋 이력 0 재확인**: 키스토어(.jks)·key.properties·play-service-account.json은 전 히스토리에 없음(gitignore+커밋 전 시크릿 스캔 방어). 유일 사본 백업 생성(`_shared/backup_secrets.sh` → Desktop tar, 사용자 iCloud/USB 복사 대기).
- 🟡 **L: google-services.json이 public repo 초기 커밋(1c01843) 히스토리에 잔존** — HEAD에선 제거됐으나 히스토리에 남음. 실질 위험 낮음: 이 파일은 APK에 동봉되어 디컴파일로 누구나 획득 가능한 "공개 취급" 설정(구글 공식 입장)이고 방어선은 RTDB 규칙(2026-06-15~17 하드닝 배포됨). 히스토리 세탁은 실익 없어 미실시. 선택 권고: Cloud 콘솔에서 Android API 키를 패키지명+SHA-1로 제한.
- 🟡 **M: FileVault OFF** — 디스크 미암호화. 맥 분실 시 키스토어·서비스계정 키 평문 노출. ~~켜기 권장~~ → **2026-07-12 사용자 수용(도난 위험 없음 판단) — 종결.**
- ℹ️ **repo private 전환 검토(2026-07-12 사용자 질문)**: 전환 자체는 가능하나 **gh-pages 웹 데모(doonghwi.github.io/cowboy-party, 라이브 200 확인·스토어 문구 '웹 데모'로 링크됨)가 무료 플랜에선 public repo 전용**이라 전환 시 웹 버전이 죽음. 시크릿은 이미 전부 git 밖(전 히스토리 0건)이라 private의 보안 이득은 미미 → **public 유지 권고**. 굳이 소스 비공개가 필요해지면: 빌드 산출물만 별도 public repo로 분리(gh-pages URL 변경 수반, 스토어·랜딩 링크 교체 필요).
- 🟡 **L: GEMINI_API_KEY가 채팅에 1회 노출**(대화 기록 잔존). 현재 무료 티어(결제 미연결)라 금전 피해 불가 — **결제 연결 직전에 재발급(rotate)** 하면 완전 해소.
- 🟡 **L(bintage): serve.py 0.0.0.0:8000 바인딩** — 공용 와이파이에서 같은 네트워크 타인이 대시보드 접근 가능. bintage 세션에 Tailscale IP 바인딩(`--host 100.x`) 권고 전달.
- **Play 서비스계정 키 유출 시 대응 경로**: Cloud 콘솔 → IAM → 서비스계정 → 키 삭제·재발급(즉시 무효화). 권한은 카우보이 앱 1개·출시 관리만으로 제한돼 있음.
