...
## 2026-07-30(21차) 애플 로그인 성공 확인 → 반려 회신 가이드 최신화(빌드 35)
- 사용자가 실기기에서 **애플 로그인 성공 확인**. ASC API로 재검증: **빌드 27~35 전부 processingState=VALID**(TestFlight 인식 여부와 무관하게 심사 제출엔 문제없음) — 최신은 35.
- `growth/APP_REVIEW_REPLY_0723.md` 최신화: 회신문 빌드 번호 30→35, 사용자가 헷갈려한 "메일이 아니라 ASC 내 Resolution Center 메시지창"이라는 점 명시, 회신 작성·영상 첨부(애플 공개 API 없음 — 사용자 필수)와 빌드 선택·심사 제출(세션이 API로 대행 가능)을 단계별로 분리 설명.
- 다음: 사용자가 회신+첨부를 ASC 웹에서 완료하면, "재제출 해줘" 요청 시 빌드 35 선택 + 심사 제출을 API로 대행.

## 2026-07-30(20차) TestFlight 끼임 확진 — 애플 백엔드가 빌드 32 이후 신규 빌드 미인식
- 장애 이론 철회(장애는 7/26, 32 업로드 전 종료). 33·34·35 전수 점검: VALID·1.0.0 트레인 소속·배포 서명 완벽(get-task-allow=False·beta-reports-active=True·스토어 프로파일)·컴플라이언스 OK — **그런데 buildBetaDetails 미생성**. 우회 전부 실패: 그룹→빌드 연결 404, 빌드 PATCH 409, **새 내부 그룹을 만들어도 32까지만 보임** → TestFlight 하위 시스템의 앱 단위 끼임.
- 대기 감시 상시 가동(tf_watch.py). 24h+ 지속 시 애플 개발자 지원 문의(코드 111 유형). **중요: 재제출은 TestFlight 무관** — 심사에는 빌드 35를 직접 첨부 가능. 관건은 애플 로그인 검증뿐인데, **서버 clientId 수정은 빌드 32에도 적용**되므로 32로 재검 가능. 사용자 에러 원문 확보가 최우선.
- 베타 그룹 '팀 테스트 2' 생성됨(진단용) — 나중에 정리.
