...
- **실기기 제보 2차 반영(07-12 오후, 4건)**: ①**"다 나갔다 동시 재입장" 착시** — 앱은 `kPresenceGraceMs`(14초) 넘게 하트비트 없는 좌석을 숨기는데 봇이 턴 제출 때만 seen을 찍어 긴 턴·결과 화면에서 전원 증발해 보였음 → `_gameLoop` 5초 주기 하트비트 + SocialSim은 결과 화면에도 매 틱 하트비트. ②**"선택 안 하면 한꺼번에 나감"** — 전 봇이 같은 30초 한계로 동시 포기 → 봇별 랜덤 한계(30~45초) + **기다리는 상대가 접속 중(하트비트 신선)이면 최대 120초 인내**(`_awaitedSeatConnected`). ③**"준비 안 눌렀는데 시작"** — 15초 grace 자동시작 제거(살아있는 사람 전원 준비 필수). ④**"내가 방장인데 멋대로 시작"** — 앱 규약(방장=최저 좌석, `_bossUid`)을 러너가 따름: **사람이 상석이면 봇은 시작·리셋·방장인수 금지**, 사람 방장이 시작한 판엔 봇이 참전(started 브랜치, gameNo로 중복참전 방지)하고 끝나면 **다시하기(pressRematch)를 눌러줌**(전원 rematch여야 앱이 resetBoard로 다음 판 진행 — 봇이 안 누르면 영영 안 넘어감). 이미 시작된 방 hostStartGame 재호출 방지. 해산 시 사람 남아있으면 방 보존(봇만 퇴장). 검증: 준비 없이 45초 시작 0회 / 준비 시 멀티라운드 / 게임 중 seen age <14초.
- **ready 값 규약 정합(07-13/14)**: 앱 v19(5b47c24)가 준비 값을 true→**좌석 주인 uid**로 바꿨는데 러너가 `==true`만 인정 → **v19+ 사람의 준비를 못 알아봐 봇 방이 전혀 시작 안 되던 사고**. 러너 `_humansReady`를 앱 computeView와 동일하게(true 또는 좌석 주인 uid 인정), 봇 setReady도 uid 기록으로. 교훈=공유 노드 값 규약 변경은 러너도 같은 커밋에서.
- **봇 지능 v2(07-14, 사용자 제보 — cpu_ai.dart는 앱이 원본, sync_core로 러너 동기화)**: ①**연속 방어(거북이) 감지**(`_defendStreak`, lastMoves 시그니처 비교로 오프라인 중복집계 방지) ②**무한 방어 봉쇄** — 전원 거북이면 일반탄 아끼고 슈퍼를 향해 장전(스킬 비례), 풀탄창 시 슈퍼 확률 0.70+공격성(거북이 있으면 0.92)·거북이 최우선 표적 ③**슈퍼 실사용** — 완급 조절(탄 2+일 때 스킬 비례로 모으기)로 6발 도달 가능해짐 ④**표적 점수화**(스킬 비례) — 무장 위협+직전 장전 가산, 연속 방어 감점, 노이즈로 사람 냄새(분산 랜덤은 유지) ⑤**관전 속도**(러너) — 사람이 방에 있는데 생존자가 전부 봇이면 3.2~6.5초/수로 감속(사람 전멸 후 봇들이 순식간에 끝내던 것), 자체전(사람 없음)은 빠르게. botUids는 호출부(matchmaker/social)에서 주입.
- **봇 자체전 BotWars(07-12, `lib/bot_wars.dart`)**: **사람이 어딘가에서 게임 중일 때만**(started 방에 하트비트 신선한 사람 존재, 15초 폴링) 봇 3~5명이 **비공개 방**(public:false — 로비에 안 보이고 사람 입장 불가)에서 자기들끼리 한 판 → 승자가 주간 랭킹 점수(기존 _recordWin 경로). 사람이 아무도 게임 중이 아니면 봇 점수 정지(랭킹 활력이 실제 활동과 비례). 동시 1판 + 판 사이 30~90초 휴식 + 예비 보존(reserve=빠른시작4+사회성4)으로 점수 인플레·봇 고갈 방지. 새 참가자 수용·로비 북적임은 SocialSim 공개방이 그대로 담당. config: botWar*.
- **주의**: 앱 규칙 바꾸면 `bash tool/sync_core.sh`+`dart test`(재현 테스트 6개). 봇 랭킹 반영은 실유저 늘면 `bot_client._recordWin` 가드로 끄기(README). **러너는 launchd KeepAlive라 pkill로 안 죽는다** — 코드 교체 후 `launchctl kickstart -k gui/$(id -u)/com.doonghwi.cowboy-bot-runner`.
